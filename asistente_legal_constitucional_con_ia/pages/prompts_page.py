...
# --- 1. Modelo de Datos para cada Prompt ---
...
class AppState(rx.State):
    ...
    # This is now a synchronous event handler
    ...
    return rx.card(
        rx.vstack(
            ...
            rx.text_area(
                ...
                on_change=lambda new_content: AppState.handle_prompt_change(phase_key, index, new_content),
            ...
            rx.hstack(
                rx.cond(
                    AppState.copied_feedback.get(prompt_unique_id, False),
                ...
                rx.button(
                    ...
                    # Call the synchronous event handler
                    on_click=AppState.copy_to_clipboard_and_show_feedback(prompt.content, phase_key, index),
                    size="2", 
                ),
                width="100%",
                margin_top="1em",
        ...
        width="100%", 
        style={"border": "1px solid #3B82F6", "border_radius": "var(--radius-3)"} # Added blue border
    )

@require_login
def prompts_page() -> rx.Component:
    """La página principal que muestra todas las fases y sus prompts."""
    return rx.container(
        rx.el.button(
            "← Regresar a pantalla principal",
            on_click=rx.redirect("/"), 
            class_name="mb-6 mt-4 px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white font-semibold rounded shadow"
        ),
        rx.vstack(
            rx.heading(
                "Metodología de Prompts para Análisis Constitucional con IA", 
                size="7", 
                align="center",
                margin_top="1em"
            ),
            rx.text(
                "Un prompt es una instrucción escrita que se le da a una inteligencia artificial para que genere una respuesta útil. Es como formular una pregunta bien enfocada para obtener un análisis o información precisa.",
                align="center",
                size="3",
                color_scheme="gray",
                margin_bottom="1em", 
            ...
            rx.foreach(
                ...
                lambda item_tuple: rx.vstack( 
                    ...
                    rx.divider(margin_y="2em", color_scheme="blue"), # Changed color_scheme to blue
                    width="100%",
                    spacing="4", 
                    align_items="center" 
                )
...
# --- Código existente para chat_page_with_copy (si aún es necesario) ---
@require_login
def chat_page_with_copy() -> rx.Component:
    # Ensure ChatState.chat_history is accessible and populated
    if hasattr(ChatState, 'chat_history') and ChatState.chat_history:
        chat_content_to_copy = "\n".join([f"{m['role']}: {m['content']}" for m in ChatState.chat_history])
    else:
        chat_content_to_copy = "No hay historial de chat para copiar."
        
    return rx.el.div(
        rx.heading("Copia del Chat", size="4"),
        rx.text_area(value=chat_content_to_copy, read_only=True, class_name="w-full h-48 mt-4 p-2 border rounded"),
        rx.el.button("Copiar Chat Completo", on_click=lambda: rx.set_clipboard(chat_content_to_copy), class_name="mt-2 px-4 py-2 bg-green-600 hover:bg-green-700 text-white rounded"),
    )
