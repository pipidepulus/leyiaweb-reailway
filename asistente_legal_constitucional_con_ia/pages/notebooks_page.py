...
def notebooks_page() -> rx.Component:
    ...
    content = rx.vstack(
        ...
        rx.hstack(
            ...
            rx.button(
                "Actualizar",
                on_click=NotebookState.load_user_notebooks,
                loading=NotebookState.loading,
                variant="outline"
            ),
            width="100%",
            align="center",
            ...
            rx.cond(
                ...
                rx.vstack(
                    rx.foreach(
                        ...
                        lambda notebook: notebook_card(notebook)
                    ),
                    spacing="4",
                    width="100%"
                ),
        ...
        rx.cond(
            ...
            rx.callout.root(
                ...
                rx.callout.text(NotebookState.error_message),
                color_scheme="red",
                margin_top="1rem"
            ),
            rx.fragment()
        ...
        rx.vstack(
            ...
            rx.hstack(
                rx.vstack(
                    ...
                    rx.text(
                        rx.cond(
                            notebook.notebook_type == "transcription",
                            "📝 Transcripción",
                            "🔍 Análisis"
                        ),
                        size="2",
                ...
                rx.hstack(
                    rx.button(
                        rx.icon("eye"),
                        "Ver",
                        on_click=rx.redirect(f"/notebooks/{notebook.id}"),
                        variant="soft",
                        size="2"
                    ),
                    rx.button(
                        rx.icon("trash-2"),
                        on_click=lambda: NotebookState.delete_notebook(notebook.id),
                        variant="soft",
                        color_scheme="red",
                        size="2"
                    ),
            ...
            rx.hstack(
                ...
                rx.cond(
                    notebook.source_data,
                    rx.text(f"Fuente: {notebook.source_data}", size="1", color="gray"),
                    rx.fragment()
                ),
                spacing="4"
            ),
            ...
            rx.text(
                "Notebook disponible para visualización",
                size="2",
                color="gray"
            ),
    ...
    content = rx.vstack(
        rx.cond(
                ...
                rx.hstack(
                    ...
                    rx.cond(
                        ...
                        rx.hstack(
                            ...
                            rx.button(
                                "❌ Cancelar",
                                on_click=NotebookState.cancel_editing,
                                variant="outline"
                            ),
                            spacing="2"
                        ),
                        rx.hstack(
                            rx.button(
                                "✏️ Editar",
                                on_click=NotebookState.start_editing,
                                color_scheme="blue"
                            ),
                            rx.button(
                                "� Copiar",
                                on_click=rx.set_clipboard(NotebookState.current_notebook_content),
                                color_scheme="green"
                            ),
                            spacing="2"
                        )
            ...
            rx.center(
                rx.vstack(
                    rx.spinner(size="3"),
                    rx.text("Cargando notebook...", color="gray"),
                    spacing="3"
                ),
                height="400px"
            )
